...
// Pre-Defining Types to use in the Code
type Student_types = {
    "Name": string,
    "Student_ID": number,
    "Course": string,
...
// input for name
let student_name: { Name: string } = await inquirer.prompt({
    type: "input",
    name: "Name",
...
for (let i in data) {
    if ((student_name.Name).toUpperCase() === ("PIAIC" + data[i].Student_ID)) {
        ...
        } else { console.log(chalk.yellowBright("Tuition Fee:"), "Not Paid") }
