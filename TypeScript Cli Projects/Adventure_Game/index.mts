...
let delay = async (action: string, time: number = 1000, color: any = "#00FF00") => {
...
let HealthPotionNumber: number = 2
let HealthPotionHeal: number = 30

async function AdventureGame() {

    let HealthPotionDropChance: number = Math.round(Math.random() * 100)
    let enemies: string[] = ["Skeleton", "Goblin", "Assassin", "Orc"]
    let Enemy: string = enemies[Math.floor(Math.random() * enemies.length)]
    let enemyHealth = (Math.round(Math.random() * 100)) + 50
    let EnemyAttackDamage: number = Math.round(Math.random() * 10) + 10
    ...
    while (enemyHealth > 0) {
        ...
        let userChoice: { UserChoice: string } = await inquirer.prompt({
            name: "UserChoice",
            type: "list",
...
let PlayAgain = async () => {
    let choice: { PlayAgain: string } = await inquirer.prompt({
        name: "PlayAgain",
        type: "list",
