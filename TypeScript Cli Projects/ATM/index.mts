...
let Balance: number = Math.round(Math.random() * 10000)
...
async function ATM() {
    ...
    let getAmount = async (): Promise<number> => {
        let value: { Amount: number } = await inquirer.prompt({
            type: "number",
            name: "Amount",
    ...
    switch (Functionality["Choices"]) {
        ...
        case chalk.yellowBright("Deposit Cash"):
            await delay()
            let Deposit: number = await getAmount()
            Balance += Deposit
            await delay()
        ...
        case chalk.yellowBright("Withdraw Cash"):
            await delay()
            let Withdraw: number = await getAmount()
            Balance -= Withdraw
            await delay()
